...
if errorlevel 1 (
    echo [ERRO] oa_proto_adt.hbp sem oa_ace.prg - abortando build RDD antigo
    exit /b 1
)
...
"%HB_INSTALL%\bin\hbmk2.exe" oa_proto_adt.hbp -w3 -static -cpu=x86_64 -ooa_proto_adt
if errorlevel 1 (
    echo [ERRO] compilacao oa_proto_adt falhou
    exit /b 1
)

echo [OK] oa_proto_adt.exe

echo [SMOKE] ACE ADT...
rd /s /q data_adt 2>nul
oa_proto_adt.exe data_adt 2>&1 | findstr /c:"ACE API" >nul
if errorlevel 1 (
