...
static void try_server(const char* label, int srvtype) {
    ...
    run_query(hStmt,"SELECT User_Name,Group_Name FROM system.usergroupmembers WHERE Group_Name LIKE 'DB:%'","User_Name","Group_Name");
    run_query(hStmt,"SELECT User_Name,Group_Name FROM system.usergroupmembers","User_Name","Group_Name");
...
int main() {
    ...
    printf("AdsCreateSQLStatement=%p\n", pfnCreateStmt); fflush(stdout);
