...
| **M7 — SQL engine** | TBD | **M7.2 landed (`SELECT *` + single-equality `WHERE`).** `engine::sql::parse_select` parses `SELECT * FROM <table> [WHERE <col> = '<literal>']` case-insensitively. Projection lists, comparison operators other than `=`, multi-clause WHERE (AND/OR/NOT), ORDER BY, joins, aggregates, subqueries, and UDFs return `AE_PARSE_ERROR`. ABI thunks: `AdsCreateSQLStatement`, `AdsCloseSQLStatement`, `AdsPrepareSQL`, `AdsExecuteSQL`, `AdsExecuteSQLDirect` — execution opens the referenced table (alias-resolving through any attached DD) and returns its handle as the cursor. **Note:** The WHERE clause currently parses but does not yet filter the cursor; that lands in M7.3 when `Table` gains a row-predicate slot. **Pending beyond M7.3:** full Advantage SQL grammar (lexer + AST + planner + executor), xBase UDFs (LEFT, SUBSTR, CTOD, ...), AEP host for stored procedures, triggers, INSERT / UPDATE / DELETE / CREATE TABLE. |
...
- **125 doctest cases / 860 assertions passing** on Windows / MSVC 2022 Release.
...
- **Live tags:** `m0-done`, `m1-done`, `m2-done`, `m3-done`, `m3.5-done`, `m3.6-partial`, `m3.7-partial`, `m3.7-closed`, `m4-partial`, `m5-partial`, `m5.1-partial`, `m5.2-partial`, `m5.3-partial`, `m6-partial`, `m7.1-partial`, `m7.2-partial`.
