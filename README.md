...
| **M3 — Indexes** | [`2026-05-03-openads-m3-indexes.md`](docs/superpowers/plans/2026-05-03-openads-m3-indexes.md) | **Done (NTX path).** NTX read + write + create end-to-end (single-leaf inserts; multi-level split is a follow-up). `Order` + `Scope` on `Table` with index-driven `goto_top` / `goto_bottom` / `skip` / `seek_key`. 15 ACE entry points (`AdsOpenIndex` / `AdsCreateIndex` / `AdsSeek` / `AdsSetScope` / etc.). `AdsPackTable` / `AdsZapTable` stubbed pending M4 memo store. `AdsSetAOF` accepts and reports `ADS_OPTIMIZED_NONE`. **CDX index = M3.5 follow-up** (FoxPro compact format + multi-tag + bit-packed entry encoding warrants a dedicated session). |
