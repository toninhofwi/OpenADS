...
int main(int argc, char** argv) {
        ...
        bool read_ok = false;
        {
            ...
            if (!fh) {
                ...
                if (sz > 0) {
                    ...
                    auto _r1 = std::fread(addData.data(), 1, addData.size(), fh);
                    read_ok = true;
                }
                std::fclose(fh);
            }
        ...
        bool read_ok2 = false;
        {
            ...
            if (fh) {
                ...
                if (sz > 0) {
                    ...
                    auto _r2 = std::fread(addData2.data(), 1, addData2.size(), fh);
                    read_ok2 = true;
                }
                std::fclose(fh);
            }
        ...
        bool am_read_ok = false;
        {
            ...
            if (fh) {
                ...
                if (sz > 0) {
                    ...
                    auto _r3 = std::fread(amData.data(), 1, amData.size(), fh);
                    am_read_ok = true;
                }
                std::fclose(fh);
            }
    ...
    if (!trigger_timings.empty()) {
        ...
        auto load_file = [](const std::string& path, std::vector<std::uint8_t>& out) -> bool {
            ...
            long sz = std::ftell(fh);
            std::rewind(fh);
            if (sz > 0) { out.resize(static_cast<std::size_t>(sz)); auto _r4 = std::fread(out.data(), 1, out.size(), fh); }
            std::fclose(fh);
            return !out.empty();
        };
