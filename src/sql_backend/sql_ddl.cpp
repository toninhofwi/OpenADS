    ...
    switch (c.xbase_type) {
        ...
        case 'N':
            if (c.dec > 0) {
                ...
                if (d == SqlDdlDialect::Oracle) {
                    return "NUMBER(" + std::to_string(len) + "," +
                           std::to_string(c.dec) + ")";
                }
                return "REAL";
            }
        ...
        default:
            ...
            if (d == SqlDdlDialect::Oracle) {
                return "VARCHAR2(" + std::to_string(len) + ")";
            }
            return "TEXT";
    }
}
...
util::Result<std::vector<std::string>> build_alter_table_add_ddl(
    ...
    for (const auto& c : columns) {
        ...
        const char* add_kw =
            (dialect == SqlDdlDialect::Mssql ||
             dialect == SqlDdlDialect::Oracle)
                ? "ADD "
                : "ADD COLUMN ";
...
util::Result<std::string> build_drop_table_ddl(
    ...
    if (dialect == SqlDdlDialect::Oracle && if_exists) {
        const std::string qtab = quote_table(dialect, table_name);
        return "BEGIN EXECUTE IMMEDIATE 'DROP TABLE " + qtab +
