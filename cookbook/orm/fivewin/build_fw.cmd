@echo off
rem ===================================================================
rem  Builder for the ORM + FiveWin grid example (grid_orm.prg).
rem  No drive letters: everything comes from arguments / environment.
rem
rem  Usage:
rem     build_fw.cmd <openads-lib-dir>
...
rem
rem  Head-less smoke after building (no window):  grid_orm.exe /auto
rem  FiveWin is a COMMERCIAL product (FiveTech) -- you need your own license.
rem ===================================================================
setlocal EnableDelayedExpansion
if "%~1"=="" ( echo Usage: build_fw.cmd ^<openads-lib-dir^> & exit /b 2 )
set "OPENADS_LIB=%~1"
...
echo === Building grid_orm.prg (ORM + FiveWin 64-bit) ===
"%HBMK%" "%HBP%" -ogrid_orm -comp=msvc64 -cpu=x86_64 -static
if errorlevel 1 ( echo BUILD FAILED & popd & exit /b 1 )

copy /y "%OPENADS_LIB%\%OPENADS_ACELIB%.dll" . >nul 2>&1
echo === Build OK : grid_orm.exe ===
popd
endlocal
