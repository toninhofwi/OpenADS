 ...
 * Back-end: MariaDB / MySQL (mariadb://user:pass@host:port/db).
 *
 * Level: INTERMEDIATE.
 *
 * Identical CRUD to sqlite/crud_sqlite.prg -- only the connection
 * string differs. MariaDB is a client/server engine, so you point the
 * URI at a running server and a database you can create tables in.
 *
 * To run, set the URI in the environment (never hardcode credentials):
 *   set DEMO_MARIA_URI=mariadb://app:secret@127.0.0.1:3306/demo   (Windows)
 *   export DEMO_MARIA_URI=mariadb://app:secret@127.0.0.1:3306/demo (POSIX)
 * If it is unset, the example prints how to set it and exits cleanly.
 *
 * Needs an OpenADS DLL built with OPENADS_WITH_MARIADB.
...
PROCEDURE Main()
   ...
   LOCAL cUri := AllTrim( GetEnv( "DEMO_MARIA_URI" ) )

   ? "OpenADS cookbook -- ORM CRUD on MariaDB / MySQL"
   ? "engine:", hbo_Version()
   ?

   IF Empty( cUri )
      ? "Set DEMO_MARIA_URI to a MariaDB/MySQL connection string, e.g.:"
   ...
   IF ! oCn:IsOpen()
      ...
      ? "(server reachable? DLL built with OPENADS_WITH_MARIADB?)"
      ErrorLevel( 1 )
      QUIT
   ENDIF
